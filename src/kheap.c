...
u32int 
kmalloc_int (u32int sz, int align, u32int *phys)
{
    if (kheap != 0)
      {
...
static void 
expand (u32int new_size, heap_t *heap)
{
    /* Sanity check. */
...
static u32int 
contract (u32int new_size, heap_t *heap)
{
    /* Sanity check. */
    ASSERT(new_size < heap->end_address - heap->start_address);
...
static s32int 
find_smallest_hole (u32int size, u8int page_align, heap_t *heap)
{
...
void *
alloc (u32int size, u8int page_align, heap_t *heap)
{
