 ...
 *
 * kheap.h -- Contains declarations for various
 * kernel heap managing functions.
 */
...
typedef struct {
    ...
    u8int readonly;         /* Should extra pages requested by us be mappes as read only? */
} heap_t;

/* Allocate a chunck of memory, sz in size. If align == 1
